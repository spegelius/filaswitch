...
class PrusaSlic3rCodeFile(GCodeFile):
    ...
    LAYER_START_RE = re.compile(b"BEFORE_LAYER_CHANGE (\d+\.*\d*)")
    ...
    def __init__(self, logger, hw_config, tower_position, purge_lines):
        ...
        self.external_perimeter_widths = []
        self.perimeter_widths = []
        self.infill_widths = []
        self.solid_infill_widths = []
        self.top_infill_widths = []
        ...
        for cmd, comment in self.layers[0].lines:
            ...
            if b"generated by Slic3r" in comment:
                ...
                try:
                    ...
                    print(e)
            elif b"lift Z" in comment:
                if cmd and gcode.is_z_move(cmd):
                    if not current_extruder in self.extruder_values:
                        self.extruder_values[current_extruder] = Extruder(current_extruder)
                    self.extruder_values[current_extruder].z_hop = gcode.last_match

            elif b"retract for toolchange" in comment:
                if cmd and gcode.is_extruder_move(cmd):
                    if not current_extruder in self.extruder_values:
                        self.extruder_values[current_extruder] = Extruder(current_extruder)
                    self.extruder_values[current_extruder].retract = gcode.last_match[0]
                    self.extruder_values[current_extruder].retract_speed = gcode.last_match[1]
