...
class KISSlicerGCodeFile(GCodeFile):
    ...
    LAYER_START_RE = re.compile(
        b" BEGIN_LAYER_OBJECT z=(\d+\.*\d*) z_thickness=(\d+\.*\d*)"
    )
    ...
    def parse_header(self):
        ...
        for cmd, comment in self.lines:
            if cmd:
                ...
                self.extruders[current_tool].retract_speed = (
                ...
                # ; flowrate_tweak = 1
                self.extruders[current_tool].feed_rate_multiplier = float(
                    comment.split(b" = ")[1]
                )
                ...
                # ; first_layer_C = 235
                self.extruders[current_tool].temperature_setpoints[0] = int(
                    comment.split(b" = ")[1]
                )

            elif current_tool is not None and b" temperature_C =" in comment:
                # ; temperature_C = 242
                self.extruders[current_tool].temperature_setpoints[1] = int(
                    comment.split(b" = ")[1]
                )
                ...
                if comment.split(b" = ")[1] != b"1":
                    self.log.warning(
                        "KISS joint layer division is not enabled. This might cause unexpected behaviour with tower size"
                    )

        if not self.version:
            self.log.warning(
                "Could not detect KISSlicer version. Use at your own risk!"
            )
        else:
        ...
        if self.settings.machine_type == 0:
            # fix KISS xy offsets
            self.settings.origin_offset_x = (
                self.settings.origin_offset_x - self.settings.stroke_x / 2
            )
            self.settings.origin_offset_y = (
