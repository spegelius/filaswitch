...
class OctoPrint(object):
    ...
    def __init__(self, url: str, api_key: str, logger: Logger):
        ...
        self.logger = logger
        self.headers = {"X-Api-Key": api_key, "User-Agent": self.USERAGENT}
    ...
    def _get(self, path):
        ...
        req = urllib.request.Request(
            self.protocol + "://" + self.url + "/api/files/{}".format(path),
    ...
    def upload_file(self, filepath, target_path, location="local"):
        ...
        try:
            ...
            if e.code != 404:
                self.logger.error(
                    "HTTP Error {}, Octoprint folder check failed".format(e)
                )
                return
            else:
                try:
                    ...
                    self.logger.error(
                        "HTTP Error {}, Octoprint folder creation failed".format(e)
                    )
                    return
        try:
...
if __name__ == "__main__":
    ...
    o.upload_file(
        "/media/Roinaa/3DModels/_work/Prusa/Fridge/Hinge_fix_v2.gcode", "test1/test3"
    )
